...
def test_stock_entry_canonicalization(db_session):
    ...
    # Verify Ledger
    txn = db_session.query(batch.inventory_txns[0].__class__).filter_by(ref_id=stock_entry.id).first()
...
def test_dispatch_entry_canonicalization(db_session):
    ...
    # Verify Ledger
    txn = db_session.query(batch.inventory_txns[0].__class__).filter(
        batch.inventory_txns[0].__class__.ref_type == "dispatch_entry"
    ).first()
...
def test_decimal_safety(db_session):
    item = db_session.query(Item).first()
    # Batch 10kg
    create_stock_entry(db_session, StockEntryCreate(item_id=item.id, quantity=10, unit="kg", received_date=date.today()), created_by=1)
    batch = db_session.query(Batch).first()
...
def test_insufficient_stock(db_session):
    item = db_session.query(Item).first()
    create_stock_entry(db_session, StockEntryCreate(item_id=item.id, quantity=1, unit="kg", received_date=date.today()), created_by=1)
    batch = db_session.query(Batch).first()
