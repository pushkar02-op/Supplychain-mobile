...
def test_mart_scoped_identity():
    ...
    resolved_a = resolve_item_for_mart(db, mart_a.id, code="PG001", name="PREMIUM_GRAIN")
    assert resolved_a.id == item_rice.id
    
    # Verify Mart B Resolution
    resolved_b = resolve_item_for_mart(db, mart_b.id, code="PG001", name="PREMIUM_GRAIN")
...
def test_resolution_precedence():
    ...
    alias_code = MartItemAlias(mart_id=mart.id, item_id=item_code_target.id, alias_code="A1", alias_name="IgnoreMe")
    
    # Alias: Name="TestName" -> NameTarget
    alias_name = MartItemAlias(mart_id=mart.id, item_id=item_name_target.id, alias_code="B2", alias_name="TestName")
