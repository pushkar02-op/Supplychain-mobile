...
from app.services.stock_entry import create_stock_entry, update_stock_entry, delete_stock_entry
from app.services.dispatch_entry import create_dispatch_entry, delete_dispatch_entry, create_dispatch_from_order
...
def clear_data(session):
    try:
        session.query(InventoryTxn).delete()
        session.query(DispatchEntry).delete()
        session.query(RejectionEntry).delete()
...
def test_stock_entry_canonicalization(db_session):
    ...
    # Verify Ledger
    txn = db_session.query(InventoryTxn).filter(InventoryTxn.ref_id == stock_entry.id).first()
...
def test_dispatch_entry_canonicalization(db_session):
    ...
    ).order_by(InventoryTxn.id.desc()).first()
...
def test_decimal_safety(db_session):
    item = db_session.query(Item).first()
    # Batch 10kg
    create_stock_entry(db_session, StockEntryCreate(item_id=item.id, quantity=10, unit="kg", received_date=date.today(), price_per_unit=10.0, total_cost=100.0), created_by=1)
    batch = db_session.query(Batch).order_by(Batch.id.desc()).first()
...
def test_insufficient_stock(db_session):
    item = db_session.query(Item).first()
    create_stock_entry(db_session, StockEntryCreate(item_id=item.id, quantity=1, unit="kg", received_date=date.today(), price_per_unit=10.0, total_cost=10.0), created_by=1)
    batch = db_session.query(Batch).order_by(Batch.id.desc()).first()
    
    # Try dispatch 1.1 kg
    try:
        create_dispatch_entry(db_session, DispatchEntryCreate(
            item_id=item.id,
            batch_id=batch.id,
            mart_name="TestMart",
            dispatch_date=date.today(),
...
if __name__ == "__main__":
    try:
        ...
        print("1. Stock Entry Canonicalization")
        s = get_session()
        test_stock_entry_canonicalization(s)
        s.close()
        print("   PASS")
        
        print("2. Dispatch Entry Canonicalization")
        s = get_session()
        test_dispatch_entry_canonicalization(s)
        s.close()
        print("   PASS")
        
        print("3. Rejection Entry Canonicalization")
        s = get_session()
        test_rejection_entry_canonicalization(s)
        s.close()
        print("   PASS")
        
        print("4. Decimal Safety")
        s = get_session()
        test_decimal_safety(s)
        s.close()
        print("   PASS")
        
        print("5. Insufficient Stock")
        s = get_session()
        test_insufficient_stock(s)
        s.close()
        print("   PASS")
