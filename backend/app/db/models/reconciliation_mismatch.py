...
class ReconciliationMismatch(Base):
    ...
    # Context
    invoice_item_id = Column(Integer, ForeignKey("invoice_item.id"), nullable=False, index=True)
    batch_id = Column(Integer, ForeignKey("batch.id"), nullable=True, index=True) # Null if MISSING_ENTRY
    ...
    status = Column(SAEnum(MismatchStatus), default=MismatchStatus.OPEN, nullable=False, index=True)
